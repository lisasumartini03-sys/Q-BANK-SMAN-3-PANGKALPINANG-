...
st.sidebar.markdown("---")
st.sidebar.info("💡 **Aplikasi Resmi SMAN 3 Pangkalpinang:** Pilihan Jenis Asesmen (Formatif/Sumatif), Validator terintegrasi, & cetak rekap Excel ber-TTD.")
...
if menu == "🏠 Beranda":
    ...
    with m1:
        st.metric(label="Jenis Soal", value="Formatif & Sumatif", delta="Kurikulum Merdeka")
    with m2:
...
# ---------------------------------------------------------
# 3. ANALISIS KUANTITATIF (EMPIRIS - SAFE EXCEL LOADER)
# ---------------------------------------------------------
    ...
    if uploaded_file is not None:
        df_raw = None
        # SISTEM PEMBACAAN FILE SERBA BISA (ANTI-CRASH)
        if uploaded_file.name.endswith(".csv"):
            ...
            # Mencoba membaca Excel dengan berbagai engine bawaan
        ...
        if df_raw is None:
            st.error("⚠️ Gagal membaca file Excel secara otomatis. Harap simpan/convert file Anda ke format `.csv` terlebih dahulu.")
        else:
            try:
                st.write("Preview Jawaban Mentah Siswa:", df_raw.head())

                soal_cols = [c for c in df_raw.columns if "soal" in str(c).lower() or "no" in str(c).lower() or "q" in str(c).lower()]
                if not soal_cols:
