...
st.sidebar.markdown("---")
st.sidebar.info("💡 **Aplikasi Resmi SMAN 3 Pangkalpinang:** Pilihan Validator terintegrasi daftar guru & cetak rekap Excel ber-TTD.")

# ---------------------------------------------------------
# 1. HALAMAN BERANDA
# ---------------------------------------------------------
if menu == "🏠 Beranda":
    st.markdown("<div class='main-header'>🏫 Sistem Bank Soal & Uji Validitas SMAN 3 Pangkalpinang</div>", unsafe_allow_html=True)
    st.markdown("<div class='sub-header'>Uji Kualitatif (Materi, Konstruksi, Bahasa) & Uji Empiris Kuantitatif (Validitas, Reliabilitas, Kesukaran, Pembeda, Pengecoh)</div>", unsafe_allow_html=True)
    
    st.markdown("---")
    m1, m2, m3, m4 = st.columns(4)
    with m1:
        st.metric(label="Validitas Kualitatif", value="3 Aspek", delta="Materi, Konstruksi, Bahasa")
    with m2:
        st.metric(label="Import Jawaban", value="ABCDE", delta="Skoring Otomatis")
    with m3:
        st.metric(label="Analisis Empiris", value="5 Indikator", delta="Pearson, K-R20, Distraktor")
    with m4:
        st.metric(label="Lembar Pengesahan", value="TTD & NIP", delta="Waka Kurikulum & Validator")
...
# ---------------------------------------------------------
# 2. ANALISIS KUALITATIF (AIKEN'S V - MATERI, KONSTRUKSI, BAHASA)
# ---------------------------------------------------------
    ...
    # IDENTITAS SOAL
    with st.expander("📌 **A. Identitas Mata Pelajaran & Penyusun**", expanded=True):
        ...
        with col_m2:
            ...
            penyusun_sel = st.selectbox("Guru Penyusun Soal:", DAFTAR_GURU_SMAN3, index=20) # Default Lisa Sumartini, S.Pd
...
# ---------------------------------------------------------
# 3. ANALISIS KUANTITATIF (EMPIRIS - IMPORT ABCDE MEKANIS)
# ---------------------------------------------------------
    ...
    with col_k1:
        kunci_str = st.text_input("🔑 Kunci Jawaban (Pisahkan dengan koma):", value="A,B,C,D,E")
    with col_k2:
        r_tabel_val = st.number_input("Nilai r-Tabel (N=30, alpha=5%):", value=0.361, step=0.001)

    kunci_list = [x.strip().upper() for x in kunci_str.split(",")]
    ...
    if uploaded_file is not None:
        try:
            ...
            st.write("Preview Jawaban Mentah Siswa:", df_raw.head())

            soal_cols = [c for c in df_raw.columns if "soal" in c.lower() or "no" in c.lower() or "q" in c.lower()]
            if not soal_cols:
                soal_cols = df_raw.columns[1:]

            df_scores = pd.DataFrame()
            for idx, col in enumerate(soal_cols):
                kunci = kunci_list[idx] if idx < len(kunci_list) else "A"
                df_scores[f"Soal_{idx+1}"] = (df_raw[col].astype(str).str.strip().str.upper() == kunci).astype(int)

            n_siswa = len(df_scores)
            n_soal_emp = len(soal_cols)
            total_skor = df_scores.sum(axis=1)

            st.subheader("📈 Hasil Analisis Empiris Butir Soal")
            ...
            res_emp = pd.DataFrame({
                ...
                "Kunci": [kunci_list[i] if i < len(kunci_list) else "-" for i in range(n_soal_emp)],
                ...
                "Kategori DP": ["Sangat Baik" if d >= 0.4 else ("Baik" if d >= 0.3 else ("Cukup" if d >= 0.2 else "Jeje/Buruk")) for d in dp_index],
            ...
            alpha = (n_soal_emp / (n_soal_emp - 1)) * (1 - (var_item / var_total)) if var_total > 0 else 0
            ...
            dist_data = []
            for idx, col in enumerate(soal_cols):
                counts = df_raw[col].astype(str).str.strip().str.upper().value_counts(normalize=True) * 100
                dist_data.append({
                    "Soal": f"Soal {idx+1}",
                    "Option A (%)": f"{round(counts.get('A', 0), 1)}%",
                    "Option B (%)": f"{round(counts.get('B', 0), 1)}%",
                    "Option C (%)": f"{round(counts.get('C', 0), 1)}%",
                    "Option D (%)": f"{round(counts.get('D', 0), 1)}%",
                    "Option E (%)": f"{round(counts.get('E', 0), 1)}%"
                })
...
# ---------------------------------------------------------
    ...
    if not st.session_state.soal_valid_kuali.empty:
        ...
        * **Guru Penyusun:** {meta.get('Penyusun', '-')} | **TA / Semester:** {meta.get('Tahun/Semester', '-')}
        ...
        # EXPORT KE EXCEL DENGAN STRUKTUR LEMBAR TTD PENGESAHAN
        buffer = io.BytesIO()
        with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
            ...
            worksheet.merge_range('A2:E2', 'SMAN 3 PANGKALPINANG', title_format)
            
            worksheet.write('A4', 'Mata Pelajaran', header_format)
            worksheet.write('B4', f": {meta.get('Mapel', '-')}")
            worksheet.write('A5', 'Fase / Kelas', header_format)
            worksheet.write('B5', f": {meta.get('Fase/Kelas', '-')}")
            worksheet.write('A6', 'Guru Penyusun', header_format)
            worksheet.write('B6', f": {meta.get('Penyusun', '-')}")
            
            worksheet.write('A9', 'Tim Validator Ahli:', header_format)
            ...
            for i in range(min(3, len(vals))):
                col = col_letters[i]
                worksheet.write(f'{col}11', f'Validator {i+1}', bold_center)
                worksheet.write(f'{col}15', vals[i], bold_center)
                worksheet.write(f'{col}16', nips[i], center_format)
                
            # Area TTD Waka Kurikulum (Era, S.T., M.M.)
            worksheet.write('C19', 'Mengetahui,', center_format)
            worksheet.write('C20', 'Wakil Kepala Sekolah Bidang Kurikulum', bold_center)
            worksheet.write('C24', 'Era, S.T., M.M.', bold_center)
            worksheet.write('C25', 'NIP. 198012292005012012', center_format)

        st.download_button(
            label="📥 Unduh Laporan Rekap Soal Valid & Lembar TTD (.xlsx)",
            data=buffer.getvalue(),
            file_name=f"Laporan_Rekap_Validitas_{meta.get('Mapel','SMAN3')}.xlsx",
