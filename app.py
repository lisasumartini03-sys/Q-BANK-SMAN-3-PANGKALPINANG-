...
# ---------------------------------------------------------
# 3. ANALISIS KUANTITATIF (AUTO-SPLIT TITIK KOMA & KOMA)
# ---------------------------------------------------------
    ...
    if uploaded_file is not None:
        ...
        if df_raw is not None:
            # FITUR PEMECAH OTOMATIS: Jika jawaban menumpuk dipisahkan ";" dalam 1 kolom
            if len(df_raw.columns) == 1:
                single_col = df_raw.columns[0]
                # Jika baris header berisi jawaban (contoh "A;A;B;C;D;E")
                first_row_str = str(single_col)
                if ";" in first_row_str or "," in first_row_str:
                    sep_char = ";" if ";" in first_row_str else ","
                    # Anggap baris header sebagai baris data siswa 1
                    header_row = pd.DataFrame([first_row_str.split(sep_char)])
                    data_rows = df_raw[single_col].astype(str).str.split(sep_char, expand=True)
                    df_split = pd.concat([header_row, data_rows], ignore_index=True)
                    df_split.columns = [f"Soal_{i+1}" for i in range(df_split.shape[1])]
                    df_raw = df_split

            try:
                st.write("Preview Jawaban Mentah Siswa (Sudah Dipisah Otomatis):", df_raw.head())

                # Deteksi Kolom Soal
                soal_cols = df_raw.columns
                ...
                for idx, col in enumerate(soal_cols):
                    ...
                    jawaban_siswa = df_raw[col].astype(str).str.strip().str.upper()
