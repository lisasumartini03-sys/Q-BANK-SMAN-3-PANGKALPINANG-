...
# Normalisasi Nama Guru untuk Validasi Username (Huruf kecil & Tanpa Spasi/Tanda Baca)
...
# CSS Animasi & Background Latar Biru
st.markdown("""
    <style>
    @keyframes float {
...
# ---------------------------------------------------------
# 🔑 HALAMAN COVER & LOGIN (LATAR BIRU + ANIMASI + JINGLE)
# ---------------------------------------------------------
if not st.session_state.logged_in:
    st.markdown("<br>", unsafe_allow_html=True)
    
    # Pratinjau Logo SMANETA
    ...
    with col2:
        ...
        st.markdown("<div class='main-title'>Q-BANK SMANETA</div>", unsafe_allow_html=True)
        st.markdown("<div class='sub-title'>Quality Question Bank — SMA Negeri 3 Pangkalpinang</div>", unsafe_allow_html=True)

        # Cek dan Putar Audio Jingle SMANETA
        ...
        with st.form("login_form"):
            st.markdown("<h4 style='text-align: center; color: #F8FAFC;'>🔐 Login Guru SMANETA</h4>", unsafe_allow_html=True)
            username_input = st.text_input("Nama Lengkap (Huruf kecil tanpa spasi):", placeholder="contoh: lisasumartinispd")
            password_input = st.text_input("Password:", type="password", placeholder="Masukkan Password (12345)")
            ...
            if submit_login:
                ...
                # Cek Kecocokan Username Guru & Password 12345
                ...
                if (is_valid_user or user_clean in USERNAMES_GURU) and password_input == "12345":
                    ...
                    st.success("🎉 Login Berhasil! Membuka Dashboard Q-BANK SMANETA...")
                    st.rerun()
                else:
                    st.error("❌ Nama Guru atau Password salah! Pastikan password adalah 12345 dan nama sesuai daftar guru.")
...
if menu == "🏠 Beranda Utama":
    st.title("🏫 Q-BANK (Quality Question Bank) SMANETA")
    st.write("Sistem Analisis Validitas & Bank Soal Resmi **SMA Negeri 3 Pangkalpinang**.")
    
    m1, m2, m3, m4 = st.columns(4)
    with m1:
        st.metric(label="Jenis Asesmen", value="Formatif & Sumatif")
    with m2:
        st.metric(label="Input Jawaban", value="Opsi ABCDE")
    with m3:
        st.metric(label="Status Login", value="Aktif ✅")
    with m4:
        st.metric(label="Pengesahan", value="Waka Kurikulum & Validator")
...
# ---------------------------------------------------------
    ...
    st.info("Fitur cetak rekap Excel dilengkapi lembar TTD Validator dan Waka Kurikulum (Era, S.T., M.M.).")
